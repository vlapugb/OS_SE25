...
## 6. Понятие файлового дескриптора (обзор)

Хотя тема билета — GOT и PLT, экзаменатор может спросить также базовое понятие **файлового дескриптора**, связанное с POSIX.

### 6.1. Определение

**Файловый дескриптор (file descriptor, FD)** в POSIX — это:

- целое неотрицательное число (тип `int`), которым процесс обозначает **открытый файл или ресурс**;
- индекс в таблице открытых файлов **конкретного процесса**.

Дескриптор используется для всех операций ввода-вывода:

- обычные файлы;
- каталоги;
- устройства (`/dev/tty`, диски и др.);
- сокеты;
- pipe, FIFO.

### 6.2. Стандартные дескрипторы
...
- ввод с клавиатуры / перенаправлённый файл;
- вывод в терминал / файл;
- вывод сообщений об ошибках (обычно не перенаправляется вместе с stdout, если явно не указать).

### 6.3. Основные системные вызовы

Работа с файловыми дескрипторами ведётся через POSIX-системные вызовы:
...
```c
int fd = open("file.txt", O_RDONLY);  // получить дескриптор
...
- При успехе `open` возвращает **номер дескриптора** (например, 3).
- При ошибке — `-1` и устанавливается `errno`.

### 6.4. Таблица дескрипторов процесса

У каждого процесса есть таблица:

- индекс = номер FD (`0,1,2,3,...`);
- элемент = указатель на запись в таблице открытых файлов ядра (где хранятся:
  - inode/объект;
  - режимы, позиция чтения/записи, счётчик ссылок и т.п.).

Дескрипторы наследуются при `fork()`:

- ребёнок получает копию таблицы дескрипторов;
- родитель и ребёнок могут совместно использовать один и тот же открытый файл (общая позиция и т.п.).

---

## 7. Краткое резюме по билету
...
4. **Файловый дескриптор**:
   - целое число, обозначающее открытый файл/ресурс в POSIX;
   - через него работают `open`, `read`, `write`, `close`;
   - стандартные дескрипторы 0/1/2 — stdin/stdout/stderr.

Такое объяснение закрывает как вопросы по механизму динамического связывания (GOT/PLT), так и базовое понятие файлового дескриптора, часто встречающееся в экзаменационных билетах по ОС.
