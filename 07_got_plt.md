...
## Небольшой ликбез по ABI (было на парах)

ABI (Application Binary Interface) — это двоичный интерфейс приложения.
Если проще: набор правил, которые определяют, как скомпилированные программы и библиотеки взаимодействуют на уровне машинного кода.
Если API — это «какие функции есть и что они делают»,
то ABI — это как именно вызываются эти функции в бинарном виде.

ABI — это контракт между:
 • компилятором,
 • линкером,
 • операционной системой,
 • библиотеками (например, libc),
 • исполняемым файлом.

Он гарантирует, что бинарники, собранные разными компиляторами или разными версиями компилятора, будут совместимы друг с другом.
...
### Быстрый ELF-ориентир: где лежат GOT/PLT

- ELF имеет **segments** (взгляд загрузчика) и **sections** (взгляд линкера/объектного файла). Один сегмент может включать несколько секций.
- Программа описывает сегменты в **Program Header Table** (`PHDR`). В памяти она лежит в отдельном сегменте `PT_PHDR`.
- Сегмент `INTERP` указывает путь до динамического загрузчика (`/lib64/ld-linux.so*`), который займётся релокациями.
- Сами таблицы:
  - `.got` и `.got.plt` — секции с указателями, попадают в `PT_LOAD` сегменты данных.
  - `.plt` — секция с переходами, попадает в `PT_LOAD` сегмент кода.
  - Символы/релокации описаны в `.dynsym/.rela.*`, которые читает загрузчик.
Этот контекст нужен, чтобы понимать, кто и куда пишет адреса при загрузке.
