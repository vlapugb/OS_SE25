...
### 1.1. Аппаратный и программный уровни (дополнение из лекционных заметок)

- Ядро представляет сетевые карты единым интерфейсом (`eth0`, `wlan0`, `enpXsY`), независимо от того, какая «железка» стоит за ним (Realtek, Atheros и т.п.). Коннектор RJ45 — классический вариант для Ethernet.
- Приём и передача пакетов оформляются в структуры `sk_buff`, где рядом хранятся заголовки L2/L3/L4 (MAC, IP, TCP/UDP) и полезная нагрузка. В стекe lwIP аналогом служит `pbuf`.
- На загруженных интерфейсах используется модель NAPI: прерывания на входящие пакеты временно маскируются, драйвер забирает пачку кадров (budget), затем снова разрешает IRQ.
- Для создания виртуальных интерфейсов в user space открывают `/dev/net/tun` или `/dev/net/tap`. Так поднимают простые VPN или тестовые сетки, не трогая физическую карту.
