...
## **1) POSIX Shell & Utilities**

(Оболочка и набор стандартных утилит)

Это всё, что относится к командной строке и программам уровня оператора:

- **POSIX shell** (описание поведения sh)
- стандартные утилиты (ls, cp, grep, awk, sed, find, xargs, sh, make, …)    
- требования к поведению команд и их аргументов
- взаимодействие команд через пайпы, переменные окружения, редиректы, подстановки и т.д.
Эта часть — **интерфейс пользователя и сценариев (скриптов)**.

---

## **2) POSIX System Interfaces (POSIX C Library)**

  

(Системные вызовы и библиотечные функции C)

Это APIs, которые программы используют в коде:

- файловые операции (open, read, write, close)    
- процессы (fork, exec, wait)
- сигналы (signal, sigaction)
- потоки (pthread_create, pthread_join)
- сокеты (socket, bind, accept)
- таймеры, блокировки, IPC, shared memory, mmap и т.д.
Это фактически **POSIX libc** (интерфейсы glibc, musl, bionic и т.д.).