...
В широком смысле под операционной системой понимается:
> - ядро ОС
> - набор работающих поверх ядра программ и утилит, предоставляющих интерфейс для взаимодействия пользователя с компьютером. 
...
### 1.1. Аппаратный контекст и шины (по заметкам)

ОС строит свои абстракции поверх конкретного «железа», которое обычно можно упростить до схемы `GPU — CPU — RAM` с подключёнными контроллерами ввода‑вывода. Базовые примеры шин, через которые периферия общается с процессором:
- PCIe (видеокарты, сетевые карты, NVMe SSD),
- SATA (накопители),
- USB (внешние устройства),
- SPI и I2C (датчики, микроконтроллеры в встроенных системах).

Роль ОС — скрыть эту разнородность за уровнем абстракции (HAL), чтобы прикладной код работал с единым интерфейсом устройств, а не с регистрами конкретных контроллеров.
...
### 2.2. Классификация ОС (связка с лекционными заметками)

По набору поддерживаемых возможностей выделяют:
- **Однопользовательские / многопользовательские**;
- **Общего назначения / специализированные** (настольные ОС, embedded-сборки);
- **Однозадачные / многозадачные**;
- **Сетевые / несетевые**;
- **ОС реального времени (RTOS)** — с мягкими и жёсткими требованиями к дедлайнам.
...
- отслеживать состояния процессов (READY, RUNNING, WAITING, DONE).
...
- реализуются буферизация, очереди запросов, планирование операций ввода-вывода. (for geeks: struct bio, struct request, blk-mq) 
...
### 3.8. Привилегии: user space vs kernel space

- **Пользовательское пространство (user space)** работает без привилегий: программы не могут напрямую обращаться к устройствам или физической памяти.
- **Пространство ядра (kernel space)** — привилегированный режим, где реализованы драйверы, планировщик, менеджер памяти.
- Общение между ними идёт через системные вызовы (`syscall`, `ecall`, `int 0x80`), которые переключают процессор из пользовательского кольца в привилегированное (например, ring3 → ring0 на x86 или U/S/M уровни на RISC-V).
- Ядро, в свою очередь, может создавать пользовательские процессы и выделять им ресурсы, сохраняя строгую изоляцию прав.
Интерфейс:
