...
**Важно:** POSIX определяет поведение «оболочки стандарта POSIX». Реальные оболочки добавляют расширения (`[[ ]]`, арифметика `(( ))`, brace expansion и т.п.), которые **не гарантированы** в чистом `/bin/sh`. Для переносимых скриптов придерживаются возможностей, описанных в стандарте: shebang `#!/bin/sh`, базовые подстановки, `test/[ ... ]`, конвейеры и редиректы. Bash и zsh поддерживают POSIX‑режим, но по умолчанию включают расширения; fish изначально не POSIX‑совместим.
...
### Расширенные оболочки
- **bash** — соответствует POSIX‑shell, но включает расширения (массово используется как интерактивная).
- **zsh** — ориентирована на удобство и мощные интерактивные функции; совместимость с POSIX ограниченная.
- **fish** — удобный интерактивный shell, не POSIX‑совместим, поэтому скрипты для `/bin/sh` в нём не запускают без портирования.
