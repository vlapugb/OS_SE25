...
Посмотреть привязку номеров к объектам удобно через псевдофайлы `/proc/<pid>/fd/*` — там видны все активные дескрипторы процесса.
...
### 3.5. `readv()` / `writev()` — векторный ввод-вывод
...
Позволяют за один системный вызов прочитать или записать несколько буферов подряд. Это экономит переключения в ядро, когда нужно собрать/разложить данные из нескольких кусочков (заголовок + тело).
