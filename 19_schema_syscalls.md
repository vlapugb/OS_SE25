...
- Примеры:
  - x86/x86_64: `syscall` (вместо старого `int 0x80`), `sysenter`/`sysexit`.
  - RISC-V: `ecall`/`envcall` — переход в привилегированный режим.

### 7.3. Уровни привилегий и виртуализация

- **x86 кольца**: 0 — ядро, 3 — пользователь. При виртуализации появляются:
  - кольцо -1 — гипервизор (VT-x/SVM),
  - кольцо -2 — System Management Mode (SMM).
  Паравиртуализация может использовать кольцо 1/2/3 для гостя, но на практике — -1/0.

- **RISC-V режимы**:
  - Machine (M) — самый привилегированный, часто прошивка OpenSBI/firmware;
  - [Hypervisor (H)] — если включён, для гипервизора;
  - Supervisor (S) — ядро ОС;
  - User (U) — пользовательские процессы.
  Пользовательский код вызывает `ecall`, ядро/гипервизор обрабатывают в соответствии с текущим уровнем.
